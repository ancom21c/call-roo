...
def _parse_configured_adapter_names(value: str) -> tuple[str, ...]:
    names: list[str] = []
    ...
    for line in result.stdout.splitlines():
        ...
        if line.startswith("hci") and ":" in line:
            if current_name is not None:
                adapters.append((current_name, current_bus))
        ...
        if current_name is None:
            continue
        if "Bus:" in stripped:
            bus_detail = stripped.split("Bus:", 1)[1].strip()
            current_bus = bus_detail.split()[0] if bus_detail else ""

    if current_name is not None:
        adapters.append((current_name, current_bus))

    adapters.sort(key=lambda item: (0 if item[1].upper() == "USB" else 1, item[0]))
    return tuple(name for name, _ in adapters)


def resolve_bluetooth_adapter_names(config: BluetoothConfig) -> tuple[str, ...]:
...
def _ordered_adapter_names(
    ...
    if preferred is None or preferred not in candidates:
        return candidates
    return (preferred,) + tuple(name for name in candidates if name != preferred)


def _rotated_adapter_names(
    candidates: tuple[str, ...],
    start_index: int,
) -> tuple[str, ...]:
    if not candidates:
        return candidates
    pivot = start_index % len(candidates)
...
class TiMiniPrintDirectPrinter:
    ...
    def _build_command(self, image_path: Path) -> tuple[list[str], Path]:
        ...
        command = [
            ...
            self.config.mac_address,
            "--mxw01-direct",
            "--no-trim-side-margins",
            "--no-trim-top-bottom-margins",
    ...
    def _ensure_connected_locked(self):
        ...
        candidates = resolve_bluetooth_adapter_names(self.config)
        adapter_names = _rotated_adapter_names(candidates, self._next_adapter_index)
        if not adapter_names:
            raise RuntimeError(
                "No usable Bluetooth adapters are available. "
                f"Configured adapter_name={self.config.adapter_name!r} "
                f"disabled={self.config.disabled_adapter_names!r}"
            )
        ...
        for adapter_name in adapter_names:
            ...
            try:
                ...
                try:
                    ...
                    pass
                failures.append(f"{adapter_name}: {exc}")
                continue

            self._session = session
            self._active_adapter_name = adapter_name
            self._next_adapter_index = (candidates.index(adapter_name) + 1) % len(candidates)
            LOGGER.info(
                "TiMini-Print direct session connected to %s via %s",
                self.config.mac_address,
                adapter_name,
            )
            return session

        self._next_adapter_index = (self._next_adapter_index + 1) % len(candidates)
        raise RuntimeError(
            "TiMini-Print direct connection failed on all candidate adapters: "
            + "; ".join(failures)
        )
    ...
    def _print_saved_image_locked(self, image_path: Path, retry_allowed: bool) -> None:
        ...
        try:
            ...
            if source_size and source_size[0] != runtime.printer_width:
                LOGGER.warning(
                    "Composed ticket width is %spx, but TiMini-Print direct printer width is %spx. "
                    "The direct backend will resize the page.",
                    source_size[0],
                    runtime.printer_width,
                )
            elif source_size:
                LOGGER.info(
                    "Composed ticket width matches TiMini-Print direct printer width: %spx",
                    runtime.printer_width,
                )
            pages, _ = runtime.load_pages(
                str(image_path),
    ...
    def _load_runtime(self) -> TiMiniPrintDirectRuntime:
        ...
        self._runtime = TiMiniPrintDirectRuntime(
            ...
            printer_width=int(getattr(mxw01_direct, "PRINTER_WIDTH", 384)),
        )
        return self._runtime
