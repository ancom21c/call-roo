...
"""
    ...
    def test_looping_wave_player_rebuilds_missing_prepared_mp3(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            clip_path = Path(tmp) / "clip.mp3"
            clip_path.write_bytes(b"ID3")
            stale_path = Path(tmp) / "stale.wav"
            rebuilt_path = Path(tmp) / "rebuilt.wav"
            stale_path.write_bytes(b"RIFF")
            rebuilt_path.write_bytes(b"RIFF")
            observed_paths: list[str] = []
            process_started = threading.Event()
            ...
            with patch(
                "callroo_printer.audio._transcode_audio_to_wav",
                side_effect=[stale_path, rebuilt_path],
            ) as transcode:
                player = LoopingWavePlayer(clip_path=clip_path, runner=runner)
                self.assertTrue(player.prime())
                stale_path.unlink()
            ...
            self.assertEqual(observed_paths, [str(rebuilt_path)])
            self.assertEqual(
                transcode.call_args_list,
                [call(clip_path, 1.0), call(clip_path, 1.0)],
            )
    ...
    def test_one_shot_wave_player_rebuilds_missing_prepared_mp3(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            clip_path = Path(tmp) / "event.mp3"
            clip_path.write_bytes(b"ID3")
            stale_path = Path(tmp) / "stale.wav"
            rebuilt_path = Path(tmp) / "rebuilt.wav"
            stale_path.write_bytes(b"RIFF")
            ...
            with patch(
                ...
                player = OneShotWavePlayer(clip_path=clip_path, runner=runner)
                self.assertTrue(player.prime())
                stale_path.unlink()
                self.assertTrue(player.play())
                self.assertTrue(started.wait(1.0))
