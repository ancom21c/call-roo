...
class LoopingWavePlayer:
    ...
    def _ensure_commands_locked(self) -> list[list[str]] | None:
        cached_commands = _cached_commands_if_ready(
            self._commands,
            self._prepared_clip_path,
            self.clip_path,
        )
        ...
        if self.volume <= 0.0:
            LOGGER.info(
                "Looping audio playback disabled because audio.launch_sound_volume is 0.0."
            )
            return None
        ...
        try:
            ...
            self._warn_once(
                f"Failed to prepare looping launch sound {self.clip_path}: {exc}"
            )
            return None
...
def _resolve_commands(clip_path: Path, device: str | None = None) -> list[list[str]] | None:
    if not clip_path.exists():
        LOGGER.warning("Audio file not found: %s", clip_path)
        return None
...
def _cached_commands_if_ready(
    commands: list[list[str]] | None,
    prepared_clip_path: Path | None,
    source_clip_path: Path,
) -> list[list[str]] | None:
    if commands is None:
        return None
    if prepared_clip_path is not None and not prepared_clip_path.exists():
        LOGGER.info(
            "Prepared audio clip %s for %s was removed; rebuilding.",
            prepared_clip_path,
            source_clip_path,
        )
        return None
